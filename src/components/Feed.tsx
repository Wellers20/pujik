...
export function Feed({ session }: { session: any }) {
  ...
  const fetchProfileAndPosts = async (silent = false) => {
    try {
      ...
      if (feedType === 'following' && session?.user) {
        ...
        // Включаем только посты тех на кого подписаны
  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-6 sm:py-8">
      {session && (
        <Stories session={session} userProfile={userProfile} />
      )}
      ...
      {session && (
        <div className="mb-2">
          {hasProfile ? (
            ...
            <div className="bg-amber-50 dark:bg-amber-950 p-5 rounded-3xl border border-amber-200 dark:border-amber-900 text-amber-900 dark:text-amber-100 text-sm mb-6 flex items-start gap-3 transition-colors">
      ...
      {/* Feed Tabs */}
      {session && (
        <div className="flex border-b border-zinc-200 dark:border-zinc-800 mb-6 sticky top-14 bg-[#fafafa]/80 dark:bg-zinc-950/80 backdrop-blur-md z-30 pt-2 px-1 transition-colors">
          <button 
            onClick={() => setFeedType('global')}
            className={cn(
              "pb-3 px-6 text-[15px] font-bold transition-colors relative",
              feedType === 'global' ? "text-zinc-900 dark:text-zinc-50" : "text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"
            )}
          >
            Для вас
            {feedType === 'global' && (
              <motion.div layoutId="feedTabIndicator" className="absolute bottom-0 left-0 right-0 h-[3px] bg-zinc-900 dark:bg-zinc-50 rounded-t-full" />
            )}
          </button>
          ...
          <button 
            ...
            className={cn(
              "pb-3 px-6 text-[15px] font-bold transition-colors relative",
              feedType === 'following' ? "text-zinc-900 dark:text-zinc-50" : "text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200"
            )}
          >
            Подписки
            {feedType === 'following' && (
