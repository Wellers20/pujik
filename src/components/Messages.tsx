          ...
          {chats.map(chat => {
            ...
            return (
              <Link key={chat.id} to={`/chat/${partner.username}`} className="flex items-center gap-4 p-4 bg-white dark:bg-zinc-900 rounded-[28px] shadow-sm border border-zinc-100 dark:border-zinc-800 hover:border-zinc-200 dark:hover:border-zinc-700 hover:shadow-md transition-all active:scale-[0.99]">
                  <div 
                    className={cn("h-[60px] w-[60px] flex-shrink-0 rounded-full flex items-center justify-center", activeStoryUsers.has(partner.id) && "p-[2px] bg-gradient-to-tr from-yellow-400 via-rose-500 to-purple-500 cursor-pointer")}
                    onClick={(e) => {
                      if (activeStoryUsers.has(partner.id)) {
                        e.preventDefault();
                        e.stopPropagation();
                        setViewStoryForUser(partner.id);
                      }
                    }}
                  >
                    <div className={cn("w-full h-full border overflow-hidden flex items-center justify-center font-bold text-lg bg-zinc-50 dark:bg-zinc-800 text-white rounded-full shadow-sm", activeStoryUsers.has(partner.id) ? "border-2 border-white dark:border-zinc-950" : "border-zinc-200/50 dark:border-zinc-700")} style={!partner.avatar_url ? generateAvatarStyle(partner.username) : undefined}>
