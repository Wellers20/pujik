    ...
    <div className="fixed inset-0 sm:relative sm:flex sm:flex-col sm:h-[calc(100vh-6rem)] sm:mt-4 max-w-xl mx-auto bg-white dark:bg-zinc-950 sm:rounded-[32px] sm:border border-zinc-200 dark:border-zinc-800 overflow-hidden z-[100] sm:z-50 shadow-lg transition-colors flex flex-col">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md sticky top-0 z-10 transition-colors flex-shrink-0">
        ...
        <Link 
          ...
          className={cn("flex-shrink-0 rounded-full", activeStoryUsers.has(partner.id) && "p-[2px] bg-gradient-to-tr from-yellow-400 via-rose-500 to-purple-500 cursor-pointer")}
          onClick={(e) => {
        ...
        >
          <div className={cn("h-10 w-10 overflow-hidden bg-zinc-50 dark:bg-zinc-900 flex items-center justify-center font-bold text-white transition-colors rounded-full", activeStoryUsers.has(partner.id) ? "border-2 border-white dark:border-zinc-950" : "border border-zinc-200/50 dark:border-zinc-700 shadow-sm")} style={!partner.avatar_url ? generateAvatarStyle(partner.username) : undefined}>
