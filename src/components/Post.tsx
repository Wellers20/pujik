...
export function Post({ post, currentUser, onPostDeleted, isDetailView = false, adminIds = new Set(), currentUserIsAdmin = false }: { post: any, currentUser: any, onPostDeleted?: (id: string) => void, isDetailView?: boolean, adminIds?: Set<string>, currentUserIsAdmin?: boolean }) {
  ...
  const { activeStoryUsers, setViewStoryForUser } = useStories();
    ...
    const checkLikedStatus = async () => {
      ...
      try {
        const { data: bData, error: bError } = await supabase
          ...
          .eq('user_id', currentUser.id)
          .single();
        if (bData && !bError) setIsBookmarked(true);
      } catch (e) {
        // Bookmarks table might not exist yet
      }
      ...
      <article className={cn(
        ...
        <div className="p-4 sm:p-5">
          <div className="flex gap-3 relative">
            ...
            <Link 
              to={`/profile/${username}`} 
              className={cn("flex-shrink-0 self-start z-10 block hover:opacity-90 rounded-full", activeStoryUsers.has(post.user_id) && "p-[2px] bg-gradient-to-tr from-yellow-400 via-rose-500 to-purple-500")} 
              onClick={e => {
                e.stopPropagation();
                if (activeStoryUsers.has(post.user_id)) {
                  e.preventDefault();
                  setViewStoryForUser(post.user_id);
                }
              }}
            >
              <div 
                className={cn("h-[44px] w-[44px] rounded-full flex items-center justify-center font-bold text-base shadow-sm overflow-hidden bg-zinc-50", activeStoryUsers.has(post.user_id) ? "border-2 border-white dark:border-zinc-950" : "border border-zinc-200/50")}
            ...
            <div 
              className={cn("flex-1 min-w-0 pb-1", !isDetailView && "cursor-pointer")}
              onClick={() => {
                if (!isDetailView) navigate(`/post/${post.id}`);
              }}
            >
              <div className="flex justify-between items-start mb-0.5" onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-1.5 truncate">
                  <Link to={`/profile/${username}`} className="font-bold text-[15px] hover:underline text-zinc-900 dark:text-zinc-50 truncate transition-colors" onClick={e => e.stopPropagation()}>
                    {username}
                  </Link>
                ...
                <div className="relative z-20">
                  <button 
                    className="text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors p-1.5 rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800" 
                    ...
                    {showMenu && (
                      <>
                        ...
                        <motion.div 
                          ...
                          className="absolute right-0 mt-1 w-44 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl overflow-hidden z-20 py-1 transition-colors"
                        >
                          {canDelete ? (
                            <button 
                              onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(true); setShowMenu(false); }} 
                              className="w-full text-left px-4 py-2.5 text-[15px] text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 font-semibold flex items-center gap-2.5 transition-colors"
                            >
              ...
              <p className={cn("text-zinc-900 dark:text-zinc-50 transition-colors text-[16px] sm:text-[17px] leading-snug whitespace-pre-wrap mt-0.5", isDetailView && "text-[18px] sm:text-[20px] leading-relaxed")}>
              ...
              <div className="flex items-center gap-6 mt-3 md:gap-8" onClick={e => e.stopPropagation()}>
                <button
                  ...
                  className={cn(
                    ...
                    isLiked ? "text-rose-600 dark:text-rose-500" : "text-zinc-500 hover:text-rose-600 dark:hover:text-rose-400",
                    isOwnPost && "opacity-40 cursor-not-allowed hover:text-zinc-500 dark:hover:text-zinc-400"
                  )}
                >
                  <div className={cn("p-2 -ml-2 rounded-full transition-colors", isLiked ? "bg-rose-50 dark:bg-rose-500/10" : "group-hover:bg-rose-50 dark:group-hover:bg-rose-500/10")}>
                ...
                <button 
                  ...
                  className={cn(
                    "group flex items-center gap-1.5 text-sm font-medium transition-colors",
                    showComments ? "text-blue-600 dark:text-blue-400" : "text-zinc-500 hover:text-blue-600 dark:hover:text-blue-400"
                  )}
                >
                  <div className={cn("p-2 -ml-2 rounded-full transition-colors", showComments ? "bg-blue-50 dark:bg-blue-500/10" : "group-hover:bg-blue-50 dark:group-hover:bg-blue-500/10")}>
                ...
                <button 
                  ...
                  className={cn(
                    "group flex items-center gap-1.5 text-sm font-medium transition-colors cursor-pointer",
                    isBookmarked ? "text-emerald-600 dark:text-emerald-500" : "text-zinc-500 hover:text-emerald-600 dark:hover:text-emerald-400",
                    !currentUser && "opacity-40 cursor-not-allowed hover:text-zinc-500 dark:hover:text-zinc-400"
                  )}
                >
                  <div className={cn("p-2 -ml-2 rounded-full transition-colors", isBookmarked ? "bg-emerald-50 dark:bg-emerald-500/10" : "group-hover:bg-emerald-50 dark:group-hover:bg-emerald-500/10")}>
                    <Bookmark className={cn("h-5 w-5 transition-transform group-active:scale-90", isBookmarked && "fill-current")} />
                  </div>
                </button>

                <button 
                  onClick={handleShare}
                  className="group flex items-center text-sm font-medium text-zinc-500 hover:text-green-600 dark:hover:text-green-400 transition-colors ml-auto mr-1 relative"
                >
                  <AnimatePresence>
                    {copied && (
                      <motion.div 
                        ...
                        exit={{ opacity: 0 }} 
                        className="absolute left-1/2 -translate-x-1/2 bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 text-[10px] px-2 py-1 rounded-md font-bold"
                      >
                        Скопировано!
                      </motion.div>
                    )}
                  </AnimatePresence>
                  <div className="p-2 rounded-full group-hover:bg-green-50 dark:group-hover:bg-green-500/10 transition-colors flex items-center gap-2">
                    {copied ? <Check className="h-5 w-5 text-green-600 dark:text-green-400" /> : <Share className="h-5 w-5 transition-transform group-active:scale-90" />}
                  </div>
                </button>
              </div>
            </div>
        ...
        <AnimatePresence>
          {showComments && (
            <motion.div 
              ...
              className="overflow-hidden bg-zinc-50/50 dark:bg-zinc-950/20 border-t border-zinc-100 dark:border-zinc-800 rounded-b-[24px] sm:rounded-b-[32px] transition-colors"
            >
              <div className="p-4 sm:p-5 pt-3">
                {loadingComments ? (
                  <div className="flex justify-center py-4">
                    <div className="h-5 w-5 animate-spin rounded-full border-2 border-zinc-300 dark:border-zinc-700 border-t-zinc-600 dark:border-t-zinc-400"></div>
                  </div>
                ) : (
                ...
                <form onSubmit={handleSubmitComment} className="flex gap-2">
                  <input
                    ...
                    maxLength={300}
                    className="flex-1 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-full px-4 py-2 text-[14.5px] focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-500 focus:ring-1 focus:ring-zinc-400 dark:focus:ring-zinc-500 transition-all shadow-sm dark:text-zinc-50"
                  />
                  <button 
                    type="submit" 
                    disabled={!newComment.trim() || submittingComment}
                    className="bg-zinc-900 dark:bg-zinc-50 text-white dark:text-zinc-900 p-2 rounded-full h-[38px] w-[38px] flex items-center justify-center disabled:opacity-40 hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors shadow-sm"
                  >
