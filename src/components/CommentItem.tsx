  ...
  return (
    <>
      <div className="flex gap-2.5 group">
        <div 
          className={cn("flex-shrink-0 self-start mt-0.5 rounded-full", activeStoryUsers.has(comment.user_id) && "p-[2px] bg-gradient-to-tr from-yellow-400 via-rose-500 to-purple-500 cursor-pointer")}
          onClick={(e) => {
            if (activeStoryUsers.has(comment.user_id)) {
              e.preventDefault();
              e.stopPropagation();
              setViewStoryForUser(comment.user_id);
            }
          }}
        >
          <div 
            className={cn("h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs shadow-sm overflow-hidden text-white bg-zinc-50", activeStoryUsers.has(comment.user_id) ? "border-[1.5px] border-white dark:border-zinc-950" : "border border-zinc-200/50 dark:border-zinc-700")}
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[18px] rounded-tl-[4px] px-3.5 py-2 flex flex-col w-fit max-w-full shadow-sm relative transition-colors">
            <div className="flex items-center gap-1.5 mb-0.5 pr-2">
              <span className="font-bold text-[13px] text-zinc-900 dark:text-zinc-50">{cUsername}</span>
              {commentIsAdmin && <AdminBadge />}
            </div>
            <p className="text-[14px] text-zinc-800 dark:text-zinc-200 leading-snug break-words">{comment.content}</p>
          </div>
          <div className="flex items-center gap-4 mt-1.5 ml-1">
            <span className="text-[12px] text-zinc-400 dark:text-zinc-500 font-medium tracking-tight">
            ...
            <button 
              ...
              className={cn(
                ...
                isLiked ? "text-rose-600 dark:text-rose-500" : "text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100"
              )}
            >
              {isLiked ? 'Любимо' : 'Нравится'} 
              {likesCount > 0 && <span className={cn("ml-0.5", isLiked ? "text-rose-600/80 dark:text-rose-500/80" : "text-zinc-400 dark:text-zinc-500")}>· {likesCount}</span>}
            </button>
            ...
            {canDelete && (
              <button 
                ...
                disabled={isDeleting}
                className="text-zinc-300 dark:text-zinc-600 hover:text-red-500 dark:hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 active:scale-90"
                title="Удалить"
              >
