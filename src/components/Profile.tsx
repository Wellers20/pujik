      ...
      if (activeTab === 'bookmarks' && session?.user && session.user.id === profile?.id) {
        ...
        try {
          const { data } = await supabase
            ...
            .select(`
              post_id,
              posts (*, profiles(username, avatar_url, banner_url), likes(count), comments(count))
            `)
    ...
    <div className="mx-auto max-w-xl px-4 py-8 relative">
      ...
      <div className="bg-white rounded-3xl pb-6 sm:pb-8 shadow-[0_2px_12px_rgb(0,0,0,0.03)] border border-zinc-100 overflow-hidden mb-8 relative">
        ...
        <div className="px-6 md:px-8 relative">
          <div 
            className={cn("absolute -top-12 md:-top-16 left-6 md:left-8 rounded-full z-10", activeStoryUsers.has(profile.id) && "p-1 bg-gradient-to-tr from-yellow-400 via-rose-500 to-purple-500 cursor-pointer")}
            onClick={() => {
              if (activeStoryUsers.has(profile.id)) {
                setViewStoryForUser(profile.id);
              }
            }}
          >
            <div 
              className={cn("h-24 w-24 md:h-32 md:w-32 rounded-full flex-shrink-0 flex items-center justify-center text-white font-bold text-3xl shadow-sm bg-zinc-50 overflow-hidden", activeStoryUsers.has(profile.id) ? "border-[4px] md:border-[6px] border-white dark:border-zinc-950" : "border-4 border-white")}
